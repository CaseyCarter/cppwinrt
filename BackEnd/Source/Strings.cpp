...
extern char const base[] = R"( )";

extern char const base_accessors[] = R"(
namespace impl
{
    template <typename T>
    struct traits
...
extern char const base_activation[] = R"(
...
extern char const base_agile[] = R"(
...
extern char const base_array[] = R"(
namespace impl
{
...
extern char const base_async_consume[] = R"(
namespace Windows::Foundation
{
...
extern char const base_async_produce[] = R"(
namespace impl
{
    template <typename D>
...
extern char const base_await_consume[] = R"(
#ifdef WINRT_ASYNC
...
extern char const base_await_produce[] = R"(
#ifdef WINRT_ASYNC
...
extern char const base_chrono[] = R"(
namespace impl
{
...
extern char const base_collections_consume[] = R"(
...
extern char const base_collections_input[] = R"(
namespace impl
{
...
extern char const base_collections_input_iterable[] = R"(
namespace impl
{
...
extern char const base_collections_input_map_view[] = R"(
namespace impl
{
...
extern char const base_collections_input_vector_view[] = R"(
namespace impl
{
    template <typename T, typename Container>
...
extern char const base_collections_map[] = R"(
namespace impl
{
    template <typename K, typename V, typename Container>
...
extern char const base_collections_produce[] = R"(
namespace impl
{
    template <typename D>
...
extern char const base_collections_vector[] = R"(
namespace impl
{
    template <typename T, typename Container>
...
extern char const base_composable[] = R"(
...
extern char const base_com_ptr[] = R"(
...
extern char const base_delegate[] = R"(
namespace ABI::Windows::Foundation
{
...
extern char const base_dependencies[] = R"(
...
extern char const base_error[] = R"(
namespace impl
{
...
extern char const base_event_consume[] = R"(
struct event_token
{
    int64_t value{};
    event_token() = default;
...
extern char const base_event_produce[] = R"(
namespace impl
{
    template <typename T>
    struct event_array
...
extern char const base_foundation[] = R"(
namespace Windows::Foundation
{
    struct Point
    {
...
extern char const base_handle[] = R"(
namespace impl
{
    template <typename T>
...
extern char const base_implements[] = R"(
namespace impl
{
    struct marker {};
}
...
extern char const base_macros[] = R"(
#ifdef _DEBUG
...
extern char const base_modern[] = R"(
#pragma once
...
extern char const base_module[] = R"(
module winrt;
...
extern char const base_propertyvalue[] = R"(
namespace ABI::Windows::Foundation
{
...
extern char const base_reference[] = R"(
namespace Windows::Foundation
{
...
extern char const base_std_async_action[] = R"(
template <typename ... Args>
...
extern char const base_std_async_action_with_progress[] = R"(
...
extern char const base_std_async_operation[] = R"(
...
extern char const base_std_async_operation_with_progress[] = R"(
...
extern char const base_std_fire_and_forget[] = R"(
template <typename ... Args>
...
extern char const base_std_hash[] = R"(namespace winrt::impl
{
...
extern char const base_string[] = R"(
namespace impl
{
...
extern char const base_string_hstring[] = R"(
...
extern char const base_string_hstring_ref[] = R"(
...
extern char const base_string_operators[] = R"(
...
extern char const base_windows[] = R"(
namespace Windows::Foundation
{
...
extern char const ConsoleHeader[] = R"(
C++ for the Windows Runtime v%
Copyright (c) % Microsoft Corporation
)";

extern char const DatabaseGetAbiClassDeclarations[] = R"(select Name, Namespace, DefaultInterface
from Classes
where DefaultInterface <> '' 
  and Namespace = ?1
order by Name)";

extern char const DatabaseGetAbiInterfaces[] = R"(select RowId, Name, Namespace, Uuid, Delegate
from Interfaces
where Namespace = ?1
order by Name)";

extern char const DatabaseGetAbiStructures[] = R"(select RowId, Name, Namespace, ifnull(Deprecated, '')
from Structures
where RowId in (select StructureId from Fields where Category in (2, 16))
  and Namespace = ?1)";

extern char const DatabaseGetBases[] = R"(with recursive
...
extern char const DatabaseGetClassConstructorsPublic[] = R"(select Interface, Name, Composable
from ClassConstructors c 
  join Interfaces i on c.Interface = i.RowId
where ClassId = ? 
  and not Protected)";

extern char const DatabaseGetClasses[] = R"(select RowId, Name, Namespace, DefaultInterface, Activatable, ifnull(Deprecated, '')
from Classes
where Namespace = ?1
order by Name
)";

extern char const DatabaseGetClassesComposable[] = R"(select RowId, Name, Namespace, DefaultInterface
from Classes
where RowId in (select ClassId from ClassConstructors where Composable and Interface in (select InterfaceId from Methods)) 
  and Namespace = ?1
order by Name)";

extern char const DatabaseGetClassImplementations[] = R"(select Name, Namespace, DefaultInterface, replace(FullName, '::', '.')
from Classes
where Namespace = ?1
order by Name)";

extern char const DatabaseGetClassOverrideIncludes[] = R"(with recursive
...
extern char const DatabaseGetClassOverrides[] = R"(with recursive
...
extern char const DatabaseGetComposableConstructors[] = R"(select c.Interface, i.Name, m.RowId, m.Name
from ClassConstructors c 
  join Interfaces i on c.Interface = i.RowId
...
extern char const DatabaseGetDelegateDefinitions[] = R"(select i.RowId, i.Name, i.Namespace, m.RowId, ifnull(i.Deprecated, '')
from Interfaces i 
...
extern char const DatabaseGetDelegates[] = R"(select d.Name, d.Namespace, m.RowId
from Interfaces d 
...
extern char const DatabaseGetEnumerations[] = R"(select RowId, Name, Namespace, Flags, ifnull(Deprecated, '')
from Enumerations
where Namespace = ?1
order by Name)";

extern char const DatabaseGetEnumerators[] = R"(select Name, Value, ifnull(Deprecated, '')
from Enumerators
where EnumerationId = ?)";

extern char const DatabaseGetEnumeratorsFlag[] = R"(select Name, printf('%X', Value)
from Enumerators
where EnumerationId = ?)";

extern char const DatabaseGetFields[] = R"(select Name, Type, Category, ifnull(Deprecated, '')
from Fields
where StructureId = ?
order by RowId)";

extern char const DatabaseGetGenericInterfaceArguments[] = R"(select distinct DependentNamespace, replace(DependentNamespace, "::", ".") as DependentNamespaceDotNotation
from NamespaceToTypeCategoryDependency 
where Namespace=?1 
  and DependentNamespace not in ("", ?1, "Windows::Foundation::Numerics", "Windows"))";

extern char const DatabaseGetGenericInterfaces[] = R"(select Name, Namespace, Uuid, RowId, replace(Uuid, "-", "_")
...
extern char const DatabaseGetInterfaceDefinitions[] = R"(select RowId, Name, Namespace, ifnull(Deprecated, '')
from Interfaces
where not Delegate 
  and Namespace = ?1
order by Name)";

extern char const DatabaseGetInterfaceMethodDefinitions[] = R"(select i.Name, i.Namespace, m.RowId, m.Name, m.AbiName
from Interfaces i
  join Methods m on m.InterfaceId = i.rowId
where not i.Delegate 
  and i.Namespace = ?1)";

extern char const DatabaseGetInterfaceMethods[] = R"(select RowId, Name, AbiName, ifnull(Deprecated, '')
from Methods
where InterfaceId = ?)";

extern char const DatabaseGetInterfaces[] = R"(select RowId, Name, Namespace, Uuid
from Interfaces
where not Delegate 
  and Namespace = ?1
order by Name)";

extern char const DatabaseGetInterfaceTraits[] = R"(select Name, Namespace, Delegate
from Interfaces
where Namespace = ?1
order by Name)";

extern char const DatabaseGetNamespaces[] = R"(select Namespace, replace(Namespace, "::", ".") as NamespaceDotNotation
from
(
...
extern char const DatabaseGetNonAbiStructures[] = R"(select RowId, Name, Namespace
from Structures
where RowId not in (select StructureId from Fields where Category = 2 or Category = 16) 
  and Namespace = ?1)";

extern char const DatabaseGetOverrides[] = R"(select RowId, Name, Namespace
from Interfaces
where RowId in (select Interface from ClassInterfaces where Overridable) 
  and Namespace = ?1
order by Name)";

extern char const DatabaseGetParameters[] = R"(select Name, Type, ClassType, Category, Flags, TypeName
from Parameters
where MethodId = ?
order by RowId)";

extern char const DatabaseGetPublicConstructorMethods[] = R"(select c.Interface, i.Name, c.Composable, m.rowId, m.Name
from ClassConstructors c 
  join Interfaces i on c.Interface = i.RowId
...
extern char const DatabaseGetRequiredAbis[] = R"(with recursive
...
extern char const DatabaseGetRequiredClassInterfaces[] = R"(with recursive
...
extern char const DatabaseGetRequiredComposableInterfaces[] = R"(with recursive
...
extern char const DatabaseGetRequiredDefinitions[] = R"(select Namespace
from Classes
where FullName in
(
  select ClassType
...
extern char const DatabaseGetRequiredForwards[] = R"(select Namespace, replace(Namespace, "::", ".") as NamespaceDotNotation
from
(
  select TypeNamespace as Namespace
  from Parameters
...
extern char const DatabaseGetRequiredInterfaceHeaders[] = R"(with recursive
...
extern char const DatabaseGetRequiredInterfaceIncludes[] = R"(with recursive
...
extern char const DatabaseGetRequiredInterfaces[] = R"(with recursive
...
extern char const DatabaseGetRequiredParamImpls[] = R"(select TypeNamespace, replace(TypeNamespace, "::", ".") as TypeNamespaceDotNotation
from Parameters
where MethodId in
(
  select RowId
...
extern char const DatabaseGetStaticMethodDeclarations[] = R"(select c.Interface, i.Name, m.RowId, m.Name, m.AbiName, ifnull(m.Deprecated, '')
from ClassStatics c 
  join Interfaces i on c.Interface = i.RowId
  join methods m on c.Interface = m.InterfaceId
where ClassId = ?1
...
extern char const DatabaseGetStructures[] = R"(select RowId, Name, Namespace, ifnull(Deprecated, '')
from Structures
where Namespace = ?1)";

extern char const DatabaseGetUsingMethodsForClass[] = R"(with recursive
...
extern char const DatabaseGetUsingMethodsForInterface[] = R"(with recursive
...
extern char const Logo[] = R"(// C++ for the Windows Runtime v%
// Copyright (c) % Microsoft Corporation. All rights reserved.
)";

extern char const OutputError[] = R"(
Output error: % (%)
)";

extern char const PostSql[] = R"(delete from Interfaces
...
extern char const PragmaOnce[] = R"(
#pragma once
)";

extern char const PragmaWarningPop[] = R"(
WINRT_WARNING_POP
)";

extern char const PragmaWarningPush[] = R"(
WINRT_WARNING_PUSH
)";

extern char const SQLiteError[] = R"(
SQLite error: % (%)
)";

extern char const TimeCompleted[] = R"(
Completed in % seconds.
)";

extern char const Usage[] = R"(
Usage:

 <sqlite database file> <Options>

Options:

 -out     <folder>       Write output in specific folder
 -nologo                 Suppress the display of the copyright banner
 -time                   Show elapsed time
)";

extern char const WriteAbiClassDefault[] = R"(template <> struct traits<%::%> { using default_interface = %; };
)";

extern char const WriteAbiInterface[] = R"(
struct __declspec(uuid("%")) __declspec(novtable) % : %
{
%};
)";

extern char const WriteAbiInterfaceMethod[] = R"(    virtual HRESULT __stdcall %(%) = 0;
)";

extern char const WriteClassComposable[] = R"(
...
extern char const WriteClassConstructorDeclaration[] = R"(    %(%);
)";

extern char const WriteClassConstructorDefinition[] = R"(
inline %::%(%) :
    %(get_activation_factory<%, %>().%(%))
{}
)";

extern char const WriteClassConstructorDefinitionComposable[] = R"(
inline %::%(%)
{
...
extern char const WriteClassDeclaration[] = R"(
struct %WINRT_EBO % :
    %%%
{
%%%};
...
extern char const WriteClassDefaultConstructorDeclaration[] = R"(    %();
)";

extern char const WriteClassDefaultConstructorDefinition[] = R"(
inline %::%() :
    %(activate_instance<%>())
{}
)";

extern char const WriteClassImplementation[] = R"(
...
extern char const WriteClassImplementationStatic[] = R"(
template <> struct traits<%::%>
{
    static constexpr const wchar_t * name() noexcept { return L"%"; }
};
...
extern char const WriteClassNullptrConstructor[] = R"(    %(std::nullptr_t) noexcept {}
)";

extern char const WriteComposableConstructor[] = R"(
    %T(%)
    {
...
extern char const WriteDelegateDefinition[] = R"(
...
extern char const WriteDelegateForward[] = R"(template <typename T> struct impl_%;
)";

extern char const WriteDelegateProducer[] = R"(
...
extern char const WriteDelegateShims[] = R"(
template <typename L> %::%(L lambda) :
...
extern char const WriteDelegateTraits[] = R"(
template <> struct traits<%::%>
{
    using abi = ABI::%::%;
};
...
extern char const WriteEnumeration[] = R"(
enum class %%
{
%};
)";

extern char const WriteEnumerationFlag[] = R"(
enum class % : unsigned
{
%};
...
extern char const WriteEnumerator[] = R"(    % %= %,
)";

extern char const WriteEnumeratorFlag[] = R"(    % = 0x%,
)";

extern char const WriteGenericInterface[] = R"(#ifndef WINRT_GENERIC_%
...
extern char const WriteInclude[] = R"(
#include "%")";

extern char const WriteInterfaceConsumer[] = R"(
template <typename D>
...
extern char const WriteInterfaceConsumerForward[] = R"(template <typename T> struct impl_%;
)";

extern char const WriteInterfaceDefinition[] = R"(
struct % :
    Windows::Foundation::IInspectable,
    impl::consume<%>%
{
...
extern char const WriteInterfaceEventMethodDeclaration[] = R"(    using %_revoker = event_revoker<%>;
    %%_revoker %(auto_revoke_t, %) const;
)";

extern char const WriteInterfaceHash[] = R"(
template<>
...
extern char const WriteInterfaceMethodDeclaration[] = R"(    %% %(%) const;
)";

extern char const WriteInterfaceProducer[] = R"(
template <typename D>
struct produce<D, %::%> : produce_base<D, %::%>
{%};
)";

extern char const WriteInterfaceProducerVirtualMethod[] = R"(
...
extern char const WriteInterfacesEventMethodDefinition[] = R"(
template <typename D> event_revoker<%> impl_%<D>::%(auto_revoke_t, %) const
{
    return impl::make_event_revoker<D, %>(this, &ABI::%::%::remove_%, %(%));
}
)";

extern char const WriteInterfacesMethodDefinition[] = R"(
template <typename D> % impl_%<D>::%(%%)";

extern char const WriteInterfaceTraits[] = R"(
template <> struct traits<%::%>
{
    using abi = ABI::%::%;
...
extern char const WriteMethodBody[] = R"() const
{%
    check_hresult(%->%(%));%
}
)";

extern char const WriteNonAbiStructure[] = R"(using % = ABI::%::%;
)";

extern char const WriteOverride[] = R"(
...
extern char const WriteOverrideDefault[] = R"(
    % %(%)
    {
        %shim().as<%>().%(%);
    }
...
extern char const WriteRequiredInterfaces[] = R"(,
    impl::require<%, %>)";

extern char const WriteSimpleTraits[] = R"(
template <> struct traits<%::%>
{
    using abi = ABI::%::%;
};
...
extern char const WriteStaticClassDeclaration[] = R"(
struct %%
{
    %() = delete;
%};
...
extern char const WriteStaticEventMethodDeclaration[] = R"(    using %_revoker = factory_event_revoker<%>;
    %static %_revoker %(auto_revoke_t, %);
)";

extern char const WriteStaticEventMethodDefinition[] = R"(
...
extern char const WriteStaticMethodDeclaration[] = R"(    %static % %(%);
)";

extern char const WriteStaticMethodDefinition[] = R"(
inline % %::%(%)
{
    %get_activation_factory<%, %>().%(%);
}
...
extern char const WriteStructure[] = R"(
struct %%
{
%};
)";

extern char const WriteStructureDeclaration[] = R"(struct %;
)";

extern char const WriteStructureField[] = R"(    %% %;
)";

extern char const WriteUsingMethod[] = R"(    using impl_%::%;
)";
