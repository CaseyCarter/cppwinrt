...
TEST_CASE("hstring,accessors,lease")
{
    hstring from = L"abc";
    hstring_ref ref = lease<hstring>(get(from));
    REQUIRE(L"abc" == ref);

    static_assert(std::is_same<hstring_ref, decltype(lease<hstring>(get(from)))>::value, "fail");
}
