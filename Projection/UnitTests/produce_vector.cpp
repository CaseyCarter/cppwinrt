...
TEST_CASE("produce_IVectorChangedEventArgs")
{
    IVectorChangedEventArgs args = make<impl::VectorChangedEventArgs>(CollectionChange::Reset, 123);

    REQUIRE(args.CollectionChange() == CollectionChange::Reset);
    REQUIRE(args.Index() == 123);
}
...
{
    IVector<int32_t> v = make<impl::Vector<int32_t>>(std::vector<int32_t>{ 1, 2, 3 });
...
TEST_CASE("produce_IVector_HttpProgress")
{
    ...
    IVector<HttpProgress> v = make<impl::Vector<HttpProgress>>(std::vector<HttpProgress>{ make_HttpProgress(1), make_HttpProgress(2), make_HttpProgress(3) });
...
{
    IVector<JsonValue> v = make<impl::Vector<JsonValue>>(std::vector<JsonValue>{ JsonValue::CreateNumberValue(1), JsonValue::CreateNumberValue(2), JsonValue::CreateNumberValue(3) });
