...
{
    bool isEqual {};
    check_hresult(WINRT_SHIM(ICryptographicBufferStatics)->abi_Compare(get(object1), get(object2), &isEqual));
    return isEqual;
}
...
template <typename D> Windows::Storage::Streams::IBuffer impl_ICryptographicBufferStatics<D>::GenerateRandom(uint32_t length) const
{
    ...
    check_hresult(WINRT_SHIM(ICryptographicBufferStatics)->abi_GenerateRandom(length, put(buffer)));
    return buffer;
}
...
{
    hstring value;
    check_hresult(WINRT_SHIM(ICryptographicBufferStatics)->abi_EncodeToHexString(get(buffer), put(value)));
    return value;
}
...
{
    hstring value;
    check_hresult(WINRT_SHIM(ICryptographicBufferStatics)->abi_EncodeToBase64String(get(buffer), put(value)));
    return value;
}
...
{
    hstring value;
    check_hresult(WINRT_SHIM(ICryptographicBufferStatics)->abi_ConvertBinaryToString(encoding, get(buffer), put(value)));
    return value;
}
