...
struct event
{
    ...
    event_token add(const Delegate & delegate)
    {
...

        {
            ...
            // Move the old list to a temp so that it can be released when no locks are held. Assign the new list.
        ...
        if (temp_targets)
        {
            for (const agile_delegate & element : *temp_targets)
            {
                ...
                catch (const hresult_error& e)
                {
                    ...
                    // Silently catch any other hresult exception from the delegate.
                }
