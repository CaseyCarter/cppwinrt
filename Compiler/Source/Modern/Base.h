...
struct String
{
	...
	{
		String result;
		check(WindowsSubstring(get(m_handle), startIndex, put(result)));
		return result;
	}
	...
	{
		String result;
		check(WindowsSubstringWithSpecifiedLength(get(m_handle), startIndex, length, put(result)));
		return result;
	}
...
struct IUnknown
{
	...
	template <typename T>
	friend DefaultAbi<T> ** put(T & object) noexcept
	{
